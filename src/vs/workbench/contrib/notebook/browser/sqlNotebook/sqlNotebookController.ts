...
export class SqlNotebookController implements IDisposable {
	...
	private readonly _connectionStatusBarItem: IStatusbarEntryAccessor;
	...
	constructor(proxy: ExtHostNotebookKernelsShape, @IStatusbarService statusbarService: IStatusbarService) {
		...
		const statusbarName = nls.localize('changeNotebookConnection', 'Change SQL Notebook Connection');
		const entryId = 'status.notebook.changeNotebookConnection';
		this._connectionStatusBarItem = statusbarService.addEntry({
			name: statusbarName,
			text: this._disconnectedLabel,
			ariaLabel: statusbarName,
			tooltip: statusbarName,
			command: ChangeSqlNotebookConnectionAction.ID
		}, entryId, StatusbarAlignment.RIGHT);
		this._disposables.push(this._connectionStatusBarItem);
